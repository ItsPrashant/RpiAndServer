...
slaveAddr=13
slaveSerialPort='/dev/ttyACM'

for sp in range(10):
    if os.path.exists(slaveSerialPort+str(sp)):
        slaveSerialPort=slaveSerialPort+str(sp)
...
class mainWindow(Frame):
    ...
    if os.path.exists(filePath+str(today)+".csv"):
        ...
        f.write("Time,Vac1,Vac2,Vac3,Vpv1,Vpv2,Iac1,Iac2,Iac3,Ipv1,Ipv2,Pnow,Etoday,Eall,Fault Code,Upload Status\n")
    f.close() 
    deviceId=1
    Time=[]
    Vac1=0
...
####
    ...
    def makeCSVfile(self):
        ...
        if(str(self.today)==str(self.lastday)):
            ...
            self.f.write(str(self.faultData)+",0")
            self.f.write("\n")
            self.f.close()
            ...
            self.f=open(self.filePath+str(self.today)+".csv","a")
            self.f.write("Time,Vac1,Vac2,Vac3,Vpv1,Vpv2,Iac1,Iac2,Iac3,Ipv1,Ipv2,Pnow,Etoday,Eall,Fault Code,Upload Status\n")
        self.lastday=self.today
